...
public class UserServiceImpl extends BaseServiceImpl<UserMapper, User> implements IUserService {
	...
	public UserInfo userInfo(String tenantId, String account, String password) {
		...
		if (Func.isNotEmpty(user)) {
			List<String> roleAlias = baseMapper.getRoleAlias(Func.toStrArray(user.getRoleId()));
			userInfo.setRoles(roleAlias);
			Boolean hasKey=redisUtil.hasKey(account+"_permissions");
			if(!hasKey){
				List<AuthTable> tableInfo=baseMapper.getTableInfo(user.getRoleId());
				if(tableInfo!=null&&tableInfo.size()>0){
					...
					while (iterator.hasNext()){
						...
						redisUtil.hset(account+"_permissions",key,permissions);
					}
				}
			}
		}
