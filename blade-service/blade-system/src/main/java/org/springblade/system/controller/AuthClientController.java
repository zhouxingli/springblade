...
import org.springblade.system.vo.ClientRoleVO;
...
public class AuthClientController extends BladeController {
	...
	*/
	...
	public R save(@Valid @RequestBody AuthClient authClient) {
		if(StringUtil.isNotBlank(authClient.getClientSecret())){
			authClient.setClientSecret(DigestUtil.encrypt(authClient.getClientSecret()));
		}
	...
	*/
	...
	public R submit(@Valid @RequestBody AuthClient authClient) {
		if(authClient.getId()==null&&StringUtil.isNotBlank(authClient.getClientSecret())){
			authClient.setClientSecret(DigestUtil.encrypt(authClient.getClientSecret()));
		}
	...
	*/
	...
	@PostMapping("/getRoleByClient")
	@ApiOperationSupport(order = 7)
	@ApiOperation(value = "拥有的角色", notes = "拥有的角色")
	public R getRoleByClient(@RequestParam Long clientId){
		List<Role> list=clientService.getRoleInfo(clientId);
		return R.data(list);
	}

	@PostMapping("/grantRole")
	@ApiOperationSupport(order = 8)
	@ApiOperation(value = "分配角色", notes = "传入clientId以及roleId集合")
	public R grantRole(@RequestBody ClientRoleVO clientRoleVO){
		boolean temp=clientService.grantRole(clientRoleVO.getClientId(),clientRoleVO.getRoleIds());
