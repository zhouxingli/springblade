...
public class RoleServiceImpl extends ServiceImpl<RoleMapper, Role> implements IRoleService {
	...
	IRoleTableService roleTableService;
	...
	@Override
	public boolean grantTable(Long roleId, List<Long> tableIds) {
		roleTableService.remove(Wrappers.<RoleTable>query().in("ROLE_ID",roleId));
		List<RoleTable> roleTables=new ArrayList<RoleTable>();
		tableIds.forEach(tableId -> {
			RoleTable roleTable=new RoleTable();
			roleTable.setRoleId(roleId);
			roleTable.setTableId(tableId);
			roleTables.add(roleTable);
		});
		return roleTableService.saveBatch(roleTables);
	}
