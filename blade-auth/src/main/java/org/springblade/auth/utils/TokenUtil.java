...
public class TokenUtil {
	...
	public final static String CLIENT_NOT_FOUND = "客户端id或密钥错误";
	...
	* 创建客户端认证的token信息
	* */
	public static AuthInfo createAuthInfo(ClientInfo clientInfo) {
		AuthClient authClient=clientInfo.getAuthClient();
		Map<String, String> param = new HashMap<>(16);
		param.put(TokenConstant.ROLE_NAME,Func.join(clientInfo.getRoles()));
		//param.put("table_name",Func.join(clientInfo.getPermissions()));
		TokenInfo accessToken=JwtUtil.createToken(param,clientInfo.getAuthClient(),"audience", "issuser", TokenConstant.ACCESS_TOKEN);
		AuthInfo authInfo = new AuthInfo();
		authInfo.setAccessToken(accessToken.getToken());
		authInfo.setExpiresIn(accessToken.getExpire());
		authInfo.setRefreshToken(createRefreshToken(clientInfo.getAuthClient()).getToken());
		authInfo.setTokenType(TokenConstant.BEARER);
		authInfo.setLicense(TokenConstant.LICENSE_NAME);
		return authInfo;
	}
	 ...
	 * @return refreshToken
	 */
	private static TokenInfo createRefreshToken(AuthClient authClient) {
		Map<String, String> param = new HashMap<>(16);
		param.put(TokenConstant.TOKEN_TYPE, TokenConstant.REFRESH_TOKEN);
		param.put(TokenConstant.CLIENT_ID, Func.toStr(authClient.getClientId()));
		return JwtUtil.createToken(param, authClient,"audience", "issuser", TokenConstant.REFRESH_TOKEN);
	}
}
